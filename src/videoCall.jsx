...
import { VideoOff, Video, Mic, MicOff, ScreenShare, PhoneOff } from "lucide-react";
    ...
    <div className="flex flex-col items-center justify-center w-screen min-h-screen bg-black text-white relative">
      ...
      <div className="absolute bottom-10 flex gap-6 bg-white rounded-xl px-6 py-4 shadow-xl text-black">
        <button onClick={toggleVideo}>
          {videoEnabled ? <Video className="w-8 h-8" /> : <VideoOff className="w-8 h-8 text-red-600" />}
        </button>
        <button onClick={toggleAudio}>
          {audioEnabled ? <Mic className="w-8 h-8" /> : <MicOff className="w-8 h-8 text-red-600" />}
        </button>
        <button onClick={handleScreenShare}>
          <ScreenShare className="w-8 h-8" />
        </button>
        <button onClick={() => {
  leaveCall(); // Optional: stop the stream
  navigate("/notificationsPro");
}}>
