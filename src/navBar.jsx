...
import RenderNotif from "./whichNotif";
...
function NavBar() {
  ...
  const profileLink = role === "pro" ? "/pro/studentProfile" : "/studentProfile";

  return (
    <nav
      className="flex w-full fixed top-0 left-0 right-0 z-10 transition-opacity duration-500 opacity-100 bg-white border-b"
    >
        ...
        <div className="flex items-center gap-10 hover:text-white-500">
          {!isBaseRoute && (
            <div className="flex items-center gap-6 px-[40px]">
              {/* Home button only if role is "company" */}
              {role === "company" && (
              ...
              {/* Show notifications only if role is NOT faculty */}
              {role !== "faculty" && (
...
<RenderNotif
              ...
              {/* Show profile only if role is NOT faculty or scad */}
              {(role !== "faculty" && role !== "scad") && (
                <div className="flex items-center gap-2">
                  <Link to={profileLink} className="transition-transform hover:scale-105 relative">
                    <div className="w-12 h-12 rounded-full bg-green-600 flex items-center justify-center">
